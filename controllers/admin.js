...
function convertUtcToIst(utcDate) {
 ...
 //    var where = await helper.findlevelId(req, res)
    //     var whereCl = ` `
    //     if (where.area_id) {
    //         var whereCl = ` area_id = ` + where.area_id
    //     }
    //     if (where.cabildo_id) {
    //         var whereCl = whereCl + ` and cabildo_id= ` + where.cabildo_id
    //     }
    //     if (where.distrito_id) {
    //         var whereCl = whereCl + ` and distrito_id= ` + where.distrito_id
    //     }
    //     if (where.group_id) {
    //         var whereCl = whereCl + ` and grupo_id= ` + where.group_id
    //     }
async function getAttendance(req, res) {
    (async () => {
        ...
        AND (:activity_id IS NULL OR activity_id = :activity_id)
        AND (:date IS NULL OR fetcha_de_actividad = :date)
        AND (:area IS NULL OR area_id = :area)
        AND (:cabildo IS NULL OR cabildo_id = :cabildo)
        AND (:district IS NULL OR distrito_id = :district)  `;

    const query = `select activity_id,area.nombre as area,fetcha_de_actividad,usuarios_actividad.nombre as activity, 
     count(activity_id) as total from attendance inner join usuarios_actividad on usuarios_actividad.id = attendance.activity_id 
    inner join usuarios_area as area on area.id = usuarios_actividad.area_id 
    ${whereClause} group by activity_id `;
...
async function  addNewMember(req,res){
    ...
    const {primer_nombre,segundo_nombre,primer_apellido,segundo_apellido,division_id,area_id,cabildo_id,distrito_sgip_id,sexo_id,grupo_id,responsable,
        fecha_nacimiento, responsable_gohonzon,nivel_responsable_id,nivel_budista_id,telefono,celular,direccion,email,cargo_responsable_id,Cedula_id,profesion_id,estado_id
    } = req.body
...
if(req.body.invitee_id){
    ...
    const query = `
      INSERT INTO usuarios_usuario
        ...
        estado_id, area_id, cabildo_id, distrito_sgip_id, grupo_id, division_id, nivel_budista_id, responsable_gohonzon)
        VALUES
        (:primer_nombre,:sexo_id,  :responsable, :nivel_responsable_id, :cargo_responsable_id, :usuario_id,:nombre_completo,:fecha_nacimiento,:fecha_ingreso,
        :primer_apellido, :segundo_nombre, :segundo_apellido, :direccion, :email, :celular, :telefono, :profesion_id,
        :estado_id, :area_id, :cabildo_id, :distrito_sgip_id, :grupo_id, :division_id, :nivel_budista_id, :responsable_gohonzon)`;
...
async function getUserDetails(req, res) {
    try {//
        ...
        let query = `select user.id, user.sgi_id ,primer_nombre , user.segundo_nombre, user.primer_apellido, user.segundo_apellido,  user.usuario_id as "Cedula_id",user.sexo_id,
...
async function reportForDivision(req, res) {
    ...
    AND (:responsable_gohonzon IS NULL OR responsable_gohonzon = :responsable_gohonzon)  `;
        ...
        return res.status(200).send({
            ...
            title: `Reorte Numerico Division`,
            data: reportResult
        });
    } catch (e) {
        console.log(e);
...
async function getGroupMemberList(req,res){
    ...
    AND (:cabildo IS NULL OR cabildo_id = :cabildo)
    AND (:district IS NULL OR distrito_sgip_id = :district)  `;
    const att = await sequelize.query(`select id from usuarios_usuario ${whereClause} AND grupo_id = ${req.body.group_id}`, { type: sequelize.QueryTypes.SELECT , replacements: {
        area: area || null,
        cabildo: cabildo || null,
        district: district || null
    }})
...
// }
        ...
        if(group_id){
            ...
            ${whereClause} and group_members.group_id = ${group_id} order by "Nombre completo"`
        }
        else{
            var query = `SELECT distinct nombre_completo as "Nombre Completo" ,email as Email,usuarios_area.nombre as Area, uc.nombre as Cabildo,ud.nombre as Distrito,usuarios_division.nombre as Division,
            responsable as "Responsable", n.nombre as "Nivel Responsable",responsable_gohonzon as "Responsable de Gohonzon",direccion as "Direccion",telefono as "Telefono",celular as "Celular" FROM usuarios_usuario  inner join usuarios_division  ON 
            ...
            left join usuarios_nivelresponsable as n on n.id = nivel_responsable_id
            inner join usuarios_cabildo as uc on uc.id = usuarios_usuario.cabildo_id inner join usuarios_distritosgip as ud on ud.id = usuarios_usuario.distrito_sgip_id
            ${whereClause} order by "Nombre completo"`;
        }
...
in (SELECT CEDULA as usuario_id FROM SUBSCRIPTION WHERE DESCRIPTION LIKE "${description}") group by area_id `
